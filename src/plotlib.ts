...
export const matplotlib_cmap = linearColorMap([128 / 2, 50 / 2, 128 / 2], [242, 255, 50], 0, 1)
...
export class coordinateSystem {
  ...
  mouse_within_bounds: boolean = false;
  ...
  circle(p: v2, r: number, color: Color) {
    ...
    const radius_in_pixels = r / (this.xMax - this.xMin) * this.ctx.canvas.width;
  ...
  heatmap(func: (x: number, y: number) => number, colorMap: (value: number) => Color, level_set: boolean, level_set_scale: number, level_set_offset: number) {
    ...
    for (let j = 0; j < this.canvas.height; j++) {
      for (let i = 0; i < this.canvas.width; i++) {
        const x = this.xMin + ((0.5+i) / this.canvas.width) * xRange;
        const y = this.yMax - ((0.5+j) / this.canvas.height) * yRange;
